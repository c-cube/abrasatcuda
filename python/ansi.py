...
def inRed( s, bold = False, pos = "foreground", blink = False ):
    args = []
    if bold:
        args.append( "bold" )
    if blink:
        args.append( "blinkSlow" )
...
class Log:
    "classe pour logguer simplement et en couleurs"
	...
	>>> import sys
        ...
        >>> Log.log( 'pan' )
        """
        print >> self.logStream, inNormal( s, bold= bold )

    @classmethod
    def logImportant( self, s, bold=False ):
        "prints in reversed blue"
        print >> self.logStream, inBlue( s, bold=bold, pos="background" )
    ...
    def logFail( self, s ):
        ...
        print >> self.logStream, inRed( s, bold=True )
